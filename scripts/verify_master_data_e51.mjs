...
const required = [
  ...
  ["src/App_nuovo.jsx", "E5.2.2 CATALOGO CANONICO GLOBALE"],
];
for (const [file, token] of required) {
  const text = fs.readFileSync(file, "utf8");
  if (!text.includes(token)) throw new Error(`[E5.2.2] Token mancante in ${file}: ${token}`);
}
const pkg = JSON.parse(fs.readFileSync("package.json", "utf8"));
if (pkg.scripts?.prebuild) throw new Error("[E5.2.2] prebuild non deve bloccare i deploy Vercel manuali");
console.log("[FMED E5.2.2] Master Data Governance frontend: OK");
